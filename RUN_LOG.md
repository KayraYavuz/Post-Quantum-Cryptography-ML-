- 2026-09-17T00:43:13+00:00 | [WS-P6.2] Patchwise Multi-Head Self-Attention modeli (WaveformTransformer1D): ResNet1D uyumlu 2D/3D girdi sözleşmesi, örnek düzeyi boolean padding_mask (yamalamadan önce sıfırlama, tamamen maskeli token dışlama, geçerli-token ortalama havuzu), sabit sinüzoidal konum kodlaması ve tahmin yardımcısı tamamlandı. torch.nn.MultiheadAttention kullanıldı; yeni bağımlılık yok. Kapsam: projeye ait genel sentetik dalga biçimleri (sinüs/kosinüs/gürültü); kriptografik etiket, anahtar kurtarma, saldırı veya servis entegrasyonu yok. Testler: hedef 137 passed (7.59 s); tam paket 549 passed, 2 bağımlılık deprecation uyarısı (13.25 s); git diff --check başarılı. Optimizer-adım duman testi eğitim/başarım kanıtı değildir; doğrulama iddiası yok. README, docs/waveform_transformer.md, PROJECT_STATE.md güncellendi; aktif adım P6.3 TODO. Commit/push sonucu sonraki kayıtta. | state: DONE_SYNTHETIC_ATTENTION
